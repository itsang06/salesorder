...
namespace ODSaleOrder.API.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class GetApplyPromotionForOrderListController : ControllerBase
    {
        ...
        public GetApplyPromotionForOrderListController(RDOSContext dbContext, IDapperRepositories dapperRepositories, IPagingService pagingService, IHttpContextAccessor contextAccessor)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> Post(RequestApplyPromotionByOrderList inp)
        {
            ...
            try
            {
                ...
                var request = new RestRequest("tppromotion/getlistpromotionbycustomer", Method.POST, DataFormat.Json);
                ...
                {
                    ApiResponse content = JsonConvert.DeserializeObject<ApiResponse>(response.Content);
                    if (!content.IsSuccess)
                    {

                        apply.NotApplyPromotions.AddRange(inp.ProductList);
                        result.Data = apply;
                        result.Success = true;
                        return Ok(result);
                    }
                    PromotionCodes = content.Data.Where(p => p.PromotionType != "04").Select(f => f.Code)?.ToList();
        ...
        public async Task<IActionResult> GetApplyPromotionByGroup(RequestApplyPromotionByOrderList inp)
        {
            ...
            try
            {
                ...
                var cloneProduct = inp.ProductList;
                PromotionGroupResponse apply = new PromotionGroupResponse();
                apply.AppliedPromotions = new List<ApplyPromotion>();
                apply.NotApplyPromotions = new List<NotApplyPromotion>();
                TrySetSchemaName(inp.DistributorCode);
                List<string> PromotionCodes = new List<string>();
                List<string> lstProduct = new List<string>();
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    {
                        res.Add(apply);
                        apply.NotApplyPromotions.AddRange(ToNotApplyPromotions(inp.ProductList));
                        result.Data = null;
                        result.Success = true;
                        return Ok(result);
                    }
        ...
        [HttpPost]
        [Route("GetApplyDiscountPromotion")]
        public async Task<IActionResult> GetPromoCodes(RequestGetPromotionCode inp)
        {
            Sys.Common.Models.Result<ResponsePromotionCode> result = new Sys.Common.Models.Result<ResponsePromotionCode>();
            try
            {
                TrySetSchemaName(inp.DistributorCode);
                ...
                _client = new RestClient(CommonData.SystemUrl.Where(x => x.Code == "ODTpAPI").Select(x => x.Url).FirstOrDefault());
                _client.Authenticator = new JwtAuthenticator($"Rdos {_token}");
                var request = new RestRequest("tpdiscount/getdiscountbycustomer", Method.POST, DataFormat.Json);
                request.AddHeader("accept", "*/*");
                request.AddHeader("DistributorCode", inp.DistributorCode);
                request.AddHeader("Content-Type", "application/json");
                var body = @"{" + "\n" +
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    GetDiscountByCustomer content = JsonConvert.DeserializeObject<GetDiscountByCustomer>(response.Content);
                    if (!content.IsSuccess || content.Data is null)
                    {
                        result.Messages.Add("Can't get list promotion");
                        return Ok(result);
                    }
                    PromotionCode = content.Data.Scheme;
                ...
                string query = @$"SELECT * FROM ""public"".""f_getpromotiondetailsbydistributor""('{inp.DistributorCode}', ARRAY[{PromotionCode}]);";
                var trans = (List<GetPromoCode>)_dapperRepositories.Query<GetPromoCode>(query);

                if (trans.Exists(f => f.LevelOrderAmount <= inp.TotalAmount))
                {
                    var filter = trans.Where(f => f.LevelOrderAmount <= inp.TotalAmount)?.OrderBy(f => f.LevelOrderAmount)?.Min();
                    if (filter.FreePercentAmount > 0)
                    {
                        ResponsePromotionCode res = new ResponsePromotionCode()
                        {
                            LevelDesc = filter.LevelDesc,
                            LevelId = filter.LevelId,
                            LevelOrderAmount = filter.LevelOrderAmount ?? 0,
                            PromomotionId = filter.PromotionId,
                            PromotionName = filter.PromotionName,
                            TotalFreeAmount = (decimal)(inp.TotalAmount * (filter.FreePercentAmount / 100)),
                            TotalFreePercentAmount = filter.FreePercentAmount ?? 0
                        };
                        result.Data = res;
                        result.Success = true;
                    }
                    else
                    {
                        ResponsePromotionCode res = new ResponsePromotionCode()
                        {
                            ...
                            PromomotionId = filter.PromotionId,
                            PromotionName = filter.PromotionName,
                            TotalFreeAmount = filter.FreeAmount ?? 0,
                            TotalFreePercentAmount = filter.FreePercentAmount ?? 0
                        };
                        result.Data = res;
                        result.Success = true;
        ...
        public async Task<IActionResult> Get(RequestApplyPromotionByOrderList inp)
        {
            ...
            try
            {
                ...
                var body = @"{" + "\n" +
                @$"  ""SaleOrgCode"": ""{inp.SaleOrgCode}""," + "\n" +
                @$"  ""SicCode"": ""{inp.SicCode ?? "null"}""," + "\n" +
                @$"  ""CustomerCode"": ""{inp.CustomerCode ?? "null"}""," + "\n" +
                @$"  ""ShiptoCode"": ""{inp.ShiptoCode ?? "S001"}""," + "\n" +
                @$"  ""RouteZoneCode"": ""{inp.RouteZoneCode ?? "null"}""," + "\n" +
                @$"  ""DsaCode"": ""{inp.DsaCode ?? "null"}""," + "\n" +
                @$"  ""Branch"": {inp.Branch ?? "null"}," + "\n" +
                @$"  ""Region"": ""{inp.Region ?? "null"}""," + "\n" +
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    PromotionCodes = content.Data.Select(f => f.Code)?.ToList();
                ...
                string promo = string.Join(",", PromotionCodes);
                string products = string.Join(",", lstProduct);
                string query = @$"SELECT * FROM ""public"".""f_getallpromotionbyorderlist""('{inp.DistributorCode}', ARRAY[{promo}], ARRAY[{products}]);";
                ...
                query = @$"SELECT * FROM ""public"".""f_getgiftsfromallpromotion""('{inp.DistributorCode}', ARRAY[{strPromo}], ARRAY[{strArr}]);"; ;
        ...
        public async Task<IActionResult> GetApplyOrderPromotion(RequestApplyPromotionByOrderList inp)
        {
            Sys.Common.Models.Result<ApplyPromotionResponse> result = new Sys.Common.Models.Result<ApplyPromotionResponse>();
            try
            {
                ...
                List<ProductItem> NotApplyPromotions = new List<ProductItem>();
                List<ApplyPromotionByOrderList> AppliedPromotions = new List<ApplyPromotionByOrderList>();
                string query = @$"SELECT * FROM ""public"".""f_getorderpromotionsbydistributor""('{inp.DistributorCode}', ARRAY[{promo}])";
                var trans = (List<ApplyPromotionByOrderList>)_dapperRepositories.Query<ApplyPromotionByOrderList>(query);
                ...
                query = @$"SELECT * FROM ""public"".""f_getquantitiesfororderpromotion""('{jsonProduct}', ARRAY[{arr}])";
                ...
                foreach (var myType in types)
                {
                    ...
                    if(apply.NotApplyPromotions is not null && apply.NotApplyPromotions.Any())
                    {
                        foreach (var not in apply.NotApplyPromotions)
                            not.Quantity = null;
                    }
                }
                ...
                #region CheckBudget
                apply = ReCheckBudgetCustomize(apply, inp);
        ...
        private ApplyPromotionResponse ReCheckBudget(ApplyPromotionResponse promotionResponse, RequestApplyPromotionByOrderList inp)
        {
            try
            {
        ...
        private ApplyPromotionResponse ReCheckBudgetCustomize(ApplyPromotionResponse promotionResponse, RequestApplyPromotionByOrderList inp)
        {
            try
            {
                if (promotionResponse.AppliedPromotions.Any() && promotionResponse.AppliedPromotions.Exists(f => f.IsApplyBudget))
                {
                    ...
                    if (response.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        ...
                        if (content.Data.Any())
                        {
                            ...
                            {
                                int availableBudget = item.BudgetRemains ?? int.MaxValue;
                                int maxAllowed = item.CustomerBudget.HasValue ? (item.CustomerBudget == 0 ? (item.BudgetBooked ?? 0) : (item.CustomerBudget ?? 0)) : int.MaxValue;
                                if (item.BudgetBooked is not null && maxAllowed == item.BudgetBooked)
                                    maxAllowed = 0;
                                else if (item.BudgetBooked is not null && item.BudgetBooked > 0)
                                    maxAllowed = maxAllowed - (item.BudgetBooked ?? 0);
                                var thisPromo = promotionResponse.AppliedPromotions.Where(f => f.IsApplyBudget && ((f.BudgetQtyCode?.Equals(item.BudgetCode) ?? false) || (f.BudgetValueCode?.Equals(item.BudgetCode) ?? false)))?.FirstOrDefault();
                                ...
                                {
                                    promotionResponse.AppliedPromotions.Remove(thisPromo);
                                }
        ...
        private int ResizeBudgetGroup(PromotionAttributeSummary promotionResponse, int slot, RequestApplyPromotionByOrderList inp)
        {
            try
            {
        ...
        private List<PromotionGroupResponse> ReCheckBudgetGroup(List<PromotionGroupResponse> promotionResponse, RequestApplyPromotionByOrderList inp)
        {
        ...
        private List<PromotionGroupResponse> CalculatorGroupValue(List<ProductItem> products, List<PromotionAttributeSummary> promotions, RequestApplyPromotionByOrderList inp, bool isFullSKU = true)
        {
            ...
            try
            {
                ...
                foreach (var promo in promotions)
                {
                    ...
                    if (isHasRequireMin)
                    {
                        ...
                        }else  if (!isFullSKU && matchedProducts.Count > 1)
                        {
                            int sum = matchedProducts.Sum(p => p.BaseQuantity) ?? 0;
                            if(sum >= promo.TotalRequiredBaseUnit)
                                ValidPromo.Add(promo);
                        }
                    }
                    else
        ...
        private List<PromotionGroupResponse> CalculatorGroupValueForNonFullSku(List<ProductItem> products, List<ProductItem> package, List<PromotionAttributeSummary> promotions, RequestApplyPromotionByOrderList inp)
        {
            List<PromotionGroupResponse> promotionGroupResponse = new();
