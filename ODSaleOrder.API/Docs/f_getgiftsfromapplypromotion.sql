CREATE OR REPLACE FUNCTION "public"."f_getgiftsfromapplypromotion"("distributorcode" varchar, "promotionlist" _varchar, "levellist" _varchar)
...
BEGIN
	RETURN QUERY
					...
					(
							...
							AND (item."FromUnit" = tp."PackingUomId" OR tp."PackingUomId" = item."BaseUnit") AND item."ToUnit" = item."BaseUnit" AND item."ItemType" = 'STOCK' AND item."Competitor" = 'f'
							WHERE tp."ProductType" = 'Stock'
							--item."FromUnit" = item."PurchaseUnit" OR 
					),
