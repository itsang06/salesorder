CREATE OR REPLACE FUNCTION ProductivityByDayReport (
...
begin
	...
	WHERE COALESCE(soi."DistributorCode" IN (SELECT unnest(distributorCodes)), FALSE)
    ...
    RETURN QUERY
    SELECT DISTINCT ON (si."InventoryID", soi."OrderDate")
        ...
        SUM(si."ShippedBaseQuantities") OVER (PARTITION BY si."InventoryID", soi."OrderDate")::int AS "ShippedBaseQuantities"
    FROM
        temp_table soi
    INNER JOIN
...
select * from ProductivityByDayReport('1000122', '2023-01-01 00:00:00','2023-12-31 23:59:59');