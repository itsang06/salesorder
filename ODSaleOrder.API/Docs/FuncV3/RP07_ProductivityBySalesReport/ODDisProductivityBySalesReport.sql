CREATE OR REPLACE FUNCTION ODDisProductivityBySalesReport (
...
begin
	...
	SELECT s."OrderRefNumber", s."DistributorCode", s."RouteZoneID",  s."OrderDate", s."IsDeleted", s."Status"
	FROM "SO_OrderInformations" s
	WHERE s."OrderDate" >= fromDate
	     AND s."OrderDate" <= toDate
	     AND NOT (s."IsDeleted")
	     AND s."RouteZoneID" IS NOT NULL
--	     AND CASE WHEN routeZoneCode IS NULL THEN '1' ELSE s."RouteZoneID" END = CASE WHEN routeZoneCode IS NULL THEN '1' ELSE routeZoneCode end
	     AND (routeZoneCode IS NULL OR s."RouteZoneID" = routeZoneCode)
...
select * 
from ODDisProductivityBySalesReport('IT01', 'p0401241', null, 'SO_ST_OPEN,SO_ST_SHIPPING,SO_ST_WAITINGSHIPPING,SO_ST_DELIVERED,SO_ST_PARTIALDELIVERED,SO_ST_FAILED,SO_ST_CONFIRM,SO_ST_COMPLETE_DRAFT', '2024-01-01', '2024-03-16', 'osp0401241');