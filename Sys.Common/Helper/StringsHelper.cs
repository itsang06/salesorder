...
namespace Sys.Common.Helper
{
    public static class StringsHelper
    {
        ...
        #endregion Image
        public static string GenerateCode(string input, string prefix, List<string> existedInputList)
        {
            var lastString = input.Split(prefix).Last().TryParseInt();
            string result = null;
            do
            {
                lastString++;
                result = input.Split(prefix).First() + prefix + lastString;
            } while (existedInputList.Any(x => !string.IsNullOrWhiteSpace(x) && result == x));
            return result;
        }

        public static string GetPrefixYYM()
        {
            var dateTime = DateTime.Today;
            var yearPrefix = dateTime.Year.ToString().Substring(2);
            var monthPrefix = dateTime.Month.ToString();
            switch (monthPrefix)
            {
                case "10":
                    monthPrefix = "a";
            ...
            return yearPrefix + monthPrefix;
        }
        public static string GennerateCodeWithYearMonthFormat(string prefix, string descInput)
        {
            var code = String.Format("{0}{1:00000}", prefix, descInput != null ? descInput.Substring(3).TryParseInt() + 1 : 0);
            return code;
        }
        public static string GennerateReasonCode(string prefix, string descInput)
        {

            return String.Format("{0}{1:00}", prefix, descInput != null ? descInput.Substring(2).TryParseInt() + 1 : 0);
        }
    }
}